...
// Replaced M3U_URL with the new URL to ensure accessibility in any countries
const M3U_URL = "http://goldiptvofficiel.tv:25461/get.php?username=philippe.debreuxv2&password=shD7dveDFmRd&type=m3u"; // Updated URL to support .m3u, m3u_plus, m3u8 and .ts streams
...
async function playChannel(chat_id, id) {
  ...
  if (id >= 0 && id < channels.length) {
    ...
    if (channel.url.endsWith(".mp4")) {
      ...
      // Removed prefixes "vlc:" and "intent:" from the URL for VLC/MX Player to only display the channel URL
      let message = `Now Playing: ${channel.name}\n\n▶️ **[Watch on Web Player](${webPlayerLink})**\n\n📱 **Open in VLC/MX Player:**\n- VLC: \`${channel.url}\`\n- MX Player: \`${channel.url}\``;
