...
const M3U_URL = "http://goldiptvofficiel.tv:25461/get.php?username=philippe.debreuxv2&password=shD7dveDFmRd&type=m3u";
...
async function playChannel(chat_id, id) {
  ...
  if (id >= 0 && id < channels.length) {
    ...
    if (channelUrl.endsWith(".mp4")) {
      ...
      let webPlayerLink = `https://videojs-http-streaming.netlify.app/?src=${encodeURIComponent(channelUrl)}`;
      // Removed the vlc:// and intent: prefixes so only the clean URL is provided for VLC and MX Player
      let message = `Now Playing: ${channel.name}\n\n▶️ **[Watch on Web Player](${webPlayerLink})**\n\n📱 **Open in VLC/MX Player:**\n- VLC: \`${channelUrl}\`\n- MX Player: \`${channelUrl}\``;
